...
export async function runDebate(spec: DebateSpec, deps: DebateDeps): Promise<DebateResult> {
  ...
  const authorSessionId =
    spec.authorSessionId ??
    deps.launch({ cwd: spec.cwd, agent: spec.author });
  const reviewerSessionId = deps.launch({ cwd: spec.cwd, agent: spec.reviewer });
  ...
  if (spec.subject === 'plan') {
    deps.send(authorSessionId, spec.objective);
    await deps.awaitSettled(authorSessionId, TURN_TIMEOUT_MS);
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    await deps.awaitSettled(reviewerSessionId, TURN_TIMEOUT_MS);
    const critique = lastAssistantText(deps.transcript(reviewerSessionId));
    if (!critique) {
    ...
    await deps.awaitSettled(authorSessionId, TURN_TIMEOUT_MS);
    const rebuttal = lastAssistantText(deps.transcript(authorSessionId));
    if (!rebuttal) {
  ...
  deps.send(authorSessionId, verdictPrompt(ran));
  await deps.awaitSettled(authorSessionId, TURN_TIMEOUT_MS);
  const verdict = lastAssistantText(deps.transcript(authorSessionId));
