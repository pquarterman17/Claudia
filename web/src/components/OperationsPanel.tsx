...
export function OperationsPanel({ session, servers, settings }: { session: SessionSummary; servers?: McpServerInfo[]; settings?: EffectiveSettings }) {
  ...
  return <section style={{ borderTop: '1px solid #303344', marginTop: 10, paddingTop: 8 }}>
    <button type="button" className="btn-ghost" aria-expanded={open} onClick={() => setOpen(!open)}>Operations {servers?.some((s) => s.status === 'failed') ? '• attention' : ''}</button>
    {open && <div style={{ display: 'grid', gap: 7, marginTop: 8, fontSize: 11 }}>
      {(servers ?? []).length === 0 ? <span className="muted">No MCP servers reported for this session.</span> : servers?.map((server) => <div key={server.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {server.status !== 'connected' && server.status !== 'disabled' && <button className="btn-ghost" onClick={() => send({ type: 'reconnect_mcp', sessionId: session.id, serverName: server.name })}>Reconnect</button>}
        <button className="btn-ghost" onClick={() => send({ type: 'toggle_mcp', sessionId: session.id, serverName: server.name, enabled: server.status === 'disabled' })}>{server.status === 'disabled' ? 'Enable' : 'Disable'}</button>
      </div>)}
      <button className="btn-ghost" onClick={() => send({ type: 'get_effective_settings', sessionId: session.id })}>Inspect effective settings</button>
